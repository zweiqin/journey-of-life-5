<template>
	<view v-if="data" class="talent-pane-container" @click="go('/marketing-treasure-house/detail/detail?id=' + data.id)">
		<BeeAvatar :size="56" :src="common.seamingImgUrl(data.avatar)"></BeeAvatar>
		<view class="talent-info">
			<view class="info-1">
				<text class="name">
					<text>{{ isTalent ? data.name : data.nickname }}</text>
				</text>
				<view v-if="data.regionName" class="address">
					<tui-icon color="#4F4F4F" :size="16" name="gps"></tui-icon>
					<text class="address-text">{{ data.regionName }}</text>
				</view>
			</view>

			<view v-if="isTalent" class="tag">
			...
			<view v-if="!isTalent" class="tag-2">
				<BeeIcon style="margin-top: -4upx" :src="require('../../../static/square/talent/vip.png')" :size="13"></BeeIcon>
				<text>初级</text>
			</view>
			<view class="desc">
				{{ data.introduce || '无介绍' }}
