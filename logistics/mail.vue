	...
	<view class="mail">
		...
		>
			<view class="header flex" @click="changeServe('/logistics/goods-info')">
				...
				<view v-if="orderForm.goodsList.length > 0" class="total-info">
					共计：<text class="number">{{ goodsInfo.totalCounter }}</text>件，<text class="number">{{ goodsInfo.totalVolume }}</text> 方，
					<text class="number">{{ goodsInfo.totalWeight }}</text> kg
				</view>
				<view v-else class="no-data">未添加货物信息</view>
		...
		<!-- 报价 -->
		<uni-collapse v-if="orderForm.priceDetail" ref="collapse">
			<uni-collapse-item title="默认开启">
				<template #title>
					<view class="collapse-header">
						...
						>
							会员价：{{ orderForm.priceDetail.sumDiscountPrice }}
						</text>
					</view>
				</template>
				<view class="collapse-content">
					<view class="collapse-content-item">
						配送费：{{ orderForm.priceDetail.detailsVo.deliveryPrice }}
					</view>
					<view class="collapse-content-item">
						安装费：{{ orderForm.priceDetail.detailsVo.installPrice }}
					</view>
					<view class="collapse-content-item">
						小额保：{{
					...
					<view class="collapse-content-item">
						干线费：{{ orderForm.priceDetail.detailsVo.trunkLinePrice }}
					</view>
				</view>
			</uni-collapse-item>
		</uni-collapse>
     ...
     */
		...
		async getOrderQuote() {
			...
			const data = {
				...
				isPickUp: this.orderForm.serve.take === '上门提货',
				deliveryType: mapDeliveryType(this.orderForm.serve.delivery),
				isHasElevator:
					this.orderForm.consigneeInfo.isHasElevator === '有',
