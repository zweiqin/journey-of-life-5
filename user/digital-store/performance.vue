...
<template>
	<view>
		...
		<!-- 数据展示 -->
		<view v-for="(item, id) in monthList" v-show="currentTab == 0" :key="id">
		...
		<view v-for="(item, index) in weekList" v-show="currentTab == 1" :key="index">
			<view class="performance-detail">
		...
		<view v-for="(item, dayId) in dayList" v-show="currentTab == 2" :key="dayId">
			<view class="performance-detail">
	...
	methods: {
		...
		getrepetitionWeek() {
			...
			for (var i1 = 0; i1 < arr1.length; i1++) {
				var ai1 = arr1[i1]
				const month = arr1[i1].month
				const week = arr1[i1].week
		...
		getrepetitionDay() {
			...
			for (var i2 = 0; i2 < arr2.length; i2++) {
				var ai2 = arr2[i2]
				const month = arr2[i2].month
				const day = arr2[i2].day
