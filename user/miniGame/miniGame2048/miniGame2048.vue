...
<script>
	import Grid from './common/js/grid.js'
	import GameManager from './common/js/game_manager.js'
	import Tile from './common/js/tile.js'
	import Vue from 'vue'
	export default {
		data() {
			return {
		...
		methods: {
			...
			touchEnd: function(events){
				...
				if(Math.max(absX,absY) > 10){
					...
					if(data){
						let highscore = uni.getStorageSync('highScore') || 0
						highscore = Math.max(highscore, data.score)
						uni.setStorageSync('highScore',highscore)
