<template>
	<view class="up-vip-container">
		...
		<view class="item" v-for="item in vips" :key="item.label">
			...
			<view class="vip-pane">
				<image class="vip-icon"
					src="https://www.tuanfengkeji.cn:9527/jf-admin-api/admin/storage/fetch/vlwhqwdgxqsb96z63itf.png" mode="" />
			</view>
        ...
        >
          撤销申请
			...
			<view class="apply-status" v-if="item.type === 1 && storeInfo">
				...
				<!-- <view class="status">{{ mapApplyStats(storeInfo.status) }}</view> -->
				<button v-if="storeInfo.userUpgradeInfo.status === 0" class="uni-btn" @click="handleWhhithDrawApply(item.type)">
					撤销申请
				</button>

				<button class="uni-btn" style="background: #52aa44" @click="handlePay(1, 'pay')" v-if="
				...
				<button @click="handlePay(1, 'try')" style="background: #2bcddd" class="uni-btn" v-if="
					storeInfo.userUpgradeInfo.status === 2 ||
					storeInfo.userUpgradeInfo.status === 3
				">
					试用
			...
			<view class="apply-status" v-if="item.type === 2 && marketingPlannerInfo">
				...
				<!-- <view class="status">{{ mapApplyStats(storeInfo.status) }}</view> -->
				<button v-if="marketingPlannerInfo.userUpgradeInfo.status === 0" class="uni-btn"
					@click="handleWhhithDrawApply(item.type)">
					撤销申请
				</button>

				<button class="uni-btn" style="background: #52aa44" @click="handlePay(2, 'pay')" v-if="
				...
				<button @click="handlePay(2, 'try')" style="background: #2bcddd" class="uni-btn" v-if="
					marketingPlannerInfo.userUpgradeInfo.status === 2 ||
					marketingPlannerInfo.userUpgradeInfo.status === 3
				">
					试用
		...
		<view class="op">
			<button class="btn" @click="go('/user/marketing-tools/apply-vip-history')">
				申请记录
			</button>
...
.up-vip-container {
	...
	height: 100vh;
	overflow: scroll;
	background: url(https://www.tuanfengkeji.cn:9527/jf-admin-api/admin/storage/fetch/zwaljjiyz29rx6l3m5vl.png) no-repeat;
	font-size: 24upx;
	color: #3d3d3d;
