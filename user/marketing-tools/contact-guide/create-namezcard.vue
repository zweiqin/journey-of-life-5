<template>
	...
	>
		...
		<Collapse style="margin-top: 20px" title="基本信息">
			<view class="base-info">
				<view class="avatar" @click="handleUploadAvatar">
					...
					<img v-else class="avatar-img" :src="form.headPic" alt="" />
		...
		<Collapse style="margin-top: 20px" title="联系方式">
			<view v-for="item in connects" :key="item.label" class="field-wrapper">
				<view class="title">{{ item.label }}</view>
				<view class="value-wrapper">
					<input
						v-model="form[item.field]"
		...
		>
			...
			<video v-show="form.video" class="video-el" :src="form.video"></video>

			<view v-show="form.video" class="reupload" @click="uploadVidoe">
				重新上传
			</view>
		</Collapse>
