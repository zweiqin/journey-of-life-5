<template>
	<view v-if="orderInfo.info" class="order-panel">
		<JHeader width="50" height="50" title="订单详情"></JHeader>
		<view class="status" :style="statusStyles">
			{{ status ? "交易完成" : "交易待完成" }}
		</view>
		<!-- 客户信息 -->
		<view v-if="defaultAddress" class="item" style="margin: 0">
			<view v-for="item in baseInfo" :key="item.id" class="line">
		...
		<view v-else class="item flex" style="justify-content: space-between" @click="handleChooseAddress">
			请选择收货地址
      ...
      " @click="handleInputRemarks"
		...
		<!-- 代金劵使用 -->
		<view
			v-if="orderInfo.info.supportVoucher && voucherNumber" class="item flex" style="
        justify-content: space-between;
        margin-bottom: 10px;
        border-bottom: 1upx solid #d8d8d8;
      "
		>
			...
			<view class="text" style="color: #fa5555; text-align: right; margin-left: 10px; flex: 1">
				{{ voucherNumber }}
			</view>
			<radio name="12" :checked="isUserVoucher" @click="handleUseVocher" />
		</view>
		...
		<uni-popup ref="orderRemarksRef">
			<uni-popup-dialog
				ref="inputClose" mode="input" type="input" title="请输入订单备注"
