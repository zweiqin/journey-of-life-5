	...
	<view class="container">
		...
		<tui-top-dropdown :show="dropShow" :paddingbtm="110" :translatey="216" @close="btnCloseDrop">
			<scroll-view class="tui-scroll-box" scroll-y scroll-with-animation :scroll-top="scrollTop">
				...
				<view
					v-for="(item, index) in proDropData" :key="index" class="tui-drop-item" :class="[ item.selected ? 'tui-bold' : '' ]"
					:data-index="index" @tap.stop="btnSelected"
				>
					<tui-icon v-if="item.selected" name="check" :size="16" color="#5677fc" :bold="true"></tui-icon>
...
export default {
	...
	methods: {
		...
		btnSelected(e) {
			...
			const obj = this.proDropData[index]
			this.$set(obj, 'selected', !obj.selected)
		},
		reset() {
			const arr = this.proDropData
