<template>
  <view class="shopContainer">
	  ...
	  <scroll-view scroll-x="true" class="menus-wrapper" scroll-with-animation
	  	:scroll-into-view="menusItemId">
	  	<view :id="'menusItem'+item.id" v-for="(item,index) in menus" :key="item.id" class="item"
	...
	<view :class="{'stic-top': scrollTop > 158}">
		...
		<view class="tui-header">
			<view class="tui-header-top">
				<view class="tui-top-item" @tap="btnDropChange(0)">排序 <image style="width: 20rpx;height: 16rpx;margin-left: 4rpx;" src="@/static/index/shop/down.png" mode=""></image> </view>
				<view class="tui-top-item" @tap="btnDropChange(1)">排序条件 <image style="width: 20rpx;height: 16rpx;margin-left: 4rpx;" src="@/static/index/shop/down.png" mode=""></image> </view>
				<view class="tui-top-item" @tap="btnDropChange(2)">
					<view>是否新品</view> <image style="width: 20rpx;height: 16rpx;margin-left: 4rpx;" src="@/static/index/shop/down.png" mode=""></image> 
		...
		<tui-top-dropdown :show="dropShow" height="auto" :translatey="translatey" @close="btnCloseDrop">
			<scroll-view class="tui-scroll-box" scroll-y scroll-with-animation :scroll-top="scrollTop">
				<!-- <view class="tui-seizeaseat-20"></view> -->
				<view class="tui-drop-item" :class="[item.selected?'tui-bold':'']" v-for="(item,index) in proDropData.list" :key="index"
				 @tap.stop="btnSelected($event,item)" :data-index="index">
					<text class="tui-ml tui-middle">{{item.name}}</text>
					<tui-icon name="check" :size="16" color="#5677fc" :bold="true" v-if="index == proDropData.listActiveIndex"></tui-icon>
				</view>
		...
		<view class="goods-wrapper">
			<scroll-view scroll-x="true" class="goodsScoll">
				<view class="sub-menus">
					<view v-for="item in submenus" :key="item.id" class="item"
	...
	<view>
		...
		<GoodsSkeleton v-if="status === 'loading' && !goodsList.length"
			background="linear-gradient(180deg, #ffffff 0%, #f6f6f6 6%)" padding="20upx"></GoodsSkeleton>
	</view>
  </view>
</template>
