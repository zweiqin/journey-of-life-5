    ...
    >
      <button class="btn">打开小程序</button>
		...
		>
			...
			>
				<swiper-item v-for="banner in bannerList" :key="banner.id"><img class="banner-img" :src="banner.picUrl" alt="" /></swiper-item>
			</swiper>
		</view>
		...
		>
			...
			<view v-show="!isShowItemPane">
				...
				<view class="home-banner animate__animated animate__flipInX">
					<img class="img" src="https://www.tuanfengkeji.cn:9527/jf-admin-api/admin/storage/fetch/6kn9oln5hpuexgpebtrj.png" alt="" />

					<img class="vip-text" src="https://www.tuanfengkeji.cn:9527/jf-admin-api/admin/storage/fetch/xd7rf3gaapi84hj96du6.png" alt="" />
				...
				<view class="footer-home">
					<view class="footer-navs">
						<view @click="handleSwitchTab('preferential')" class="footer-nav-item" :class="{ active: currentIndex === 'preferential' }">特惠专区</view>

						<view @click="handleSwitchTab('hotGoodsList')" :class="{ active: currentIndex === 'hotGoodsList' }" class="footer-nav-item">爆品选购</view>
					</view>
            ...
            >
              ...
              <view class="goods-info">
                ...
                <view class="goods-price">￥{{ item.retailPrice }}</view>
						...
						<JGoods v-for="item in footerData" :key="item.id" :data="item" :scroll-top="scrollTop"></JGoods>
					</view>
				</view>
			</view>

			<view v-if="isShowItemPane && !noData">
				<JTabs ref="jTabsRef" @change="handleChangeCurrentPane" :activeIndex="currentActive" :tabs="labelList" class="j-tabs"></JTabs>
				...
				<!-- #ifdef APP-PLUS -->
				<view class="goods-wrapper"><JGoods v-for="item in data" :key="item.id" class="j-goods" ref="jGoodsRef" :scrollTop="scrollTop" :data="item"></JGoods></view>
				<!-- #endif -->
			</view>

			<noData v-show="!labelList.length && isShowItemPane" text="该分类下暂无商品"></noData>
		</view>
		<JAside
			@op="handleOp"
			:style="{
...
import { J_LOACTION, J_REFRSH, J_STORE_TYPES, J_TOKEN_EXPIRE, J_USER_TOKEN } from '../../constant';
...
export default {
		...
		getHomeData() {
			getIndexDataApi().then(({ data }) => {
				...
				this.bannerList = data.topicList.filter(item => {
					return item.title !== '团蜂科技开业盛典' && item.title !== '千姿万丽内衣专场';
				});
			});
		},
	...
	computed: {
		bannerComputed() {
			console.log(this.currentNav.background.slice(0, this.currentNav.background.length - 1) + ', #fff)');
			return this.currentNav.background.slice(0, this.currentNav.background.length - 1) + ', #fff)';
		}
	},
