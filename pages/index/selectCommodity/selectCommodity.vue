<template>
	<view class="selectCommodityView">
		<view class="search-header">
			<image src="../../../static/images/user/back.png" style="width: 30rpx;height: 60rpx;margin-left:20rpx;margin-right: 20rpx;" mode="" @click="goBack"></image>
		...
		<view class="selectScollView">
			<scroll-view scroll-y="true" class="superSelect">
				<view class="superSelectItem" v-for="(item,index) in menus" :key="item.id" @click="getSubItem(item,index)">
					<span :class="{ isactive:submenusActive == index }">{{ item.name }}</span>
				</view>
			</scroll-view>
			<scroll-view scroll-y="true" class="subSelect">
				<view class="subSelectItem" v-for="item in submenus" @click="goToShopList(item)">
					<image :src="item.appIconUrl?item.appIconUrl:require('@/static/index/menus/kongtupian.png')" mode=""></image>
					<text>{{ item.name }}</text>
				</view>
			</scroll-view>
		</view>
