<template>
	...
	<view class="slider_selection_container">
		<view class="AnchorList">
			<view
				v-for="(item, index) in 10" class="AnchorItem" :class="{ currter: current == index }"
		...
		<scroll-view
			class="selectionItem" scroll-with-animation scroll-with-animation scroll-y
			@scrolltolower="lower"
			@scroll="rightScroll"
		>
			<view v-for="item in 10" class="commodityItem">
				<image src="../image/cai.png" mode=""></image>
				<view class="commodity_msg">
					<p>酱爆鱿鱼丝</p>
					<p> <span>月售300+</span> <span>好评度100%</span> <span class="isDiscount">4折</span> </p>
...
export default {
	...
	methods: {
		async rightScroll(e) {
			...
			const scrollHeight = e.detail.scrollTop + this.cateBarHeight / 2
			for (let i = 0; i < this.rightScrollArr.length; i++) {
				const height1 = this.rightScrollArr[i]
				const height2 = this.rightScrollArr[i + 1]
