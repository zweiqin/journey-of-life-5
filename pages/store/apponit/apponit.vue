	...
	<view class="apponit-container">
		...
		<view class="pane form-area">
			...
			<view class="item f-f-s">
				...
				<input
					v-model="apponitForm.arrivalTime" class="font-14" readonly disabled
					type="text" placeholder="请选择到店时间"
					@click="handleChooseDate"
				>
				<tui-datetime
					ref="datetimeRef" :set-date-time="currentTime" :start-year="currentYear"
					:end-year="currentYear + 1" :type="1" @confirm="handleChooseTime"
				></tui-datetime>
			...
			<view class="item f-s-b">
				<textarea
					v-model.trim="apponitForm.remarks" placeholder="备注:(如需其他服务请备注)" class="font-12 r-5" cols="30"
					rows="10"
				></textarea>
			</view>
		</view>
		...
		<view class="footer f-center">
			<button v-if="!apponitInfo.id" class="uni-btn f-center f-w-500 f-c-ff submit" @click="submitApponit">提交预约</button>
